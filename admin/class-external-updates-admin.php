...
class External_Updates_Admin {
	...
	public function activate_licence($plugin,$key){
		...
		if(!$update_url || !$update_id){return false;}
		...
		// Call the custom API.
		$response = wp_remote_post( $update_url, array( 'timeout'   => 15,
		                                                  'sslverify' => false,
		                                                  'body'      => $api_params
		) );
	...
	public function get_plugins_for_update_by_src(){
		...
		foreach($plugins as $key => $plugin){
			if(isset($plugin['Update URL']) && $plugin['Update URL']){
				...
				// setup the updater
				$update_array = array(
					'slug'    => $key,                      // the addon slug
					'version' => $plugin['Version'],       // current version number
					'license' => isset($keys[$key]->key) ? $keys[$key]->key : '',           // license key (used get_option above to retrieve from DB)
					'item_id' => $plugin['Update ID'],    // id of this addon on GD site
					'url'     => home_url(),
	...
	public function convert_github_release( $_data, $release ){
		...
		if(!isset($_data['slug'])){return false;}
...
//		echo '###'.$_src ;
		...
		$request = wp_remote_get( $_src , array( 'timeout'   => 15,
		                                          'sslverify' => false,
		                                          'body'      => ''
		) );
	...
	public function api_request( $_action,$_src , $_data ){
		...
		$request = wp_remote_post( $_src , array( 'timeout'   => 15,
		                                                  'sslverify' => false,
		                                                  'body'      => $api_params
		) );
